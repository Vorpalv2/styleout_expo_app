...
import { ActionButton, AppHeader, radii, RoundAction, SamplePieceImage, samplePieces, SmallCaps } from '@/components/StyleoutUI';
...
export default function WardrobeScreen() {
  ...
  async function save() {
    ...
    try {
      ...
      if (editingItem) {
        await updateItem(editingItem.id, { ...changes, ...(replacementImage ? { image: replacementImage } : {}) });
        showToast(`${changes.name} updated in your wardrobe.`);
      } else {
        savedItem = await addItem({ ...changes, image: draftImage });
        showToast(`${changes.name} added to your wardrobe.`);
      }
  ...
  async function remove(id: string) {
    try { await removeItem(id); setDetail(null); showToast('Item removed from your wardrobe.', 'info'); }
    ...
    <View style={s.screen}>
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 32 }} alwaysBounceVertical refreshControl={Platform.OS === 'web' ? undefined : <ClosetRefreshControl />}>
        ...
        {shown.length ? <View style={s.grid}>{shown.map((item) => (
          ...
          <View style={s.emptyImages}>{samplePieces.filter((piece) => ['Tops', 'Bottoms', 'Shoes'].includes(piece.category)).map((piece, i) => <View key={piece.name} style={[s.sampleTile, { transform: [{ rotate: i === 0 ? '-8deg' : i === 2 ? '8deg' : '0deg' }] }]}><SamplePieceImage col={piece.col} row={piece.row} size={82} /></View>)}</View>
          ...
          <ActionButton onPress={startAdd} label="＋  Add your first piece" style={s.darkButton} />
        </View>}
      </ScrollView>
      <Modal visible={!!draftImage} animationType="slide" presentationStyle="pageSheet" onRequestClose={closeForm}>
        <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={s.formScreen}>
          <ScrollView contentContainerStyle={s.formContent} keyboardShouldPersistTaps="handled">
            ...
            <ActionButton onPress={save} disabled={!name.trim() || saving} label={saving ? 'Saving…' : editingItem ? 'Save changes' : 'Save to wardrobe'} style={s.saveButton} />
          </ScrollView>
      ...
      <Modal visible={!!detail} transparent animationType="slide" onRequestClose={() => setDetail(null)}>
        <View style={s.backdrop}><View style={s.detailSheet}>
          ...
          {detail ? <><LoadingImage source={{ uri: detail.image }} style={s.detailImage} resizeMode="contain" /><Text style={s.detailCategory}>{detail.category.toUpperCase()}</Text><Text style={s.detailTitle}>{detail.name}</Text><Text style={s.detailMeta}>{[detail.color, detail.brand].filter(Boolean).join(' · ') || 'Your piece'}</Text>{detail.notes ? <Text style={s.detailNotes}>{detail.notes}</Text> : null}<ActionButton onPress={() => startEdit(detail)} label="Edit piece and photo" style={s.editButton} /><ActionButton onPress={() => confirmRemove(detail)} label="Remove from wardrobe" tone="outline" style={s.removeButton} labelStyle={{ color: palette.danger }} /></> : null}
        </View></View>
      </Modal>
    </View>
  );
...
const makeStyles = (palette: ThemeColors) => StyleSheet.create({
  screen: { flex: 1, backgroundColor: palette.paper }, plus: { fontSize: 25, color: palette.ink, marginTop: -3 },
  ...
  grid: { flexDirection: 'row', flexWrap: 'wrap', paddingHorizontal: 18, gap: 12 }, card: { width: '48%', borderWidth: 1, borderColor: palette.line, borderRadius: radii.card, overflow: 'hidden', backgroundColor: palette.surface },
  ...
  empty: { marginHorizontal: 24, marginTop: 50, alignItems: 'center' }, emptyImages: { height: 120, flexDirection: 'row', alignItems: 'center', marginBottom: 25 }, sampleTile: { borderRadius: 16, overflow: 'hidden', borderWidth: 1, borderColor: palette.line, marginHorizontal: -5, backgroundColor: palette.surface },
  ...
  darkButton: { minWidth: 210, marginTop: 25 },
  formScreen: { flex: 1, backgroundColor: palette.paper }, formContent: { padding: 24, paddingBottom: 50 }, formHead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'flex-start', marginTop: 12, marginBottom: 20 }, formTitle: { fontSize: 29, fontWeight: '600', letterSpacing: -0.8, color: palette.ink, marginTop: 6 }, close: { fontSize: 29, color: palette.ink, lineHeight: 30 },
  ...
  input: { height: 48, borderWidth: 1, borderColor: palette.line, borderRadius: radii.control, paddingHorizontal: 15, fontSize: 14, color: palette.ink, marginBottom: 10, backgroundColor: palette.surface }, notes: { height: 78, paddingTop: 14, textAlignVertical: 'top' },
  ...
  categoryText: { fontSize: 12, color: palette.muted, fontWeight: '600' }, categoryTextActive: { color: '#fff' }, saveButton: { marginTop: 21 }, disabled: { opacity: 0.35 }, saveText: { color: '#fff', fontSize: 14, fontWeight: '600' },
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: '#0006' }, detailSheet: { padding: 24, paddingBottom: 40, borderTopLeftRadius: 26, borderTopRightRadius: 26, backgroundColor: palette.paper }, sheetHead: { flexDirection: 'row', justifyContent: 'space-between', marginBottom: 15 },
  detailImage: { width: '100%', height: 280, backgroundColor: palette.canvas, borderRadius: radii.card }, detailCategory: { fontSize: 10, fontWeight: '700', color: palette.muted, letterSpacing: 1.5, marginTop: 20 },
  detailTitle: { fontSize: 25, fontWeight: '600', color: palette.ink, marginTop: 5 }, detailMeta: { color: palette.muted, fontSize: 13, marginTop: 5 }, detailNotes: { color: palette.ink, fontSize: 13, marginTop: 14, lineHeight: 19 }, editButton: { marginTop: 22 }, editText: { color: '#fff', fontWeight: '600', fontSize: 12 }, removeButton: { marginTop: 10 }, removeText: { color: palette.danger, fontWeight: '600', fontSize: 12 },
});
