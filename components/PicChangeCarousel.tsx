...
export function PicChangeCarousel({ visible, onClose }: Props) {
  ...
  async function choose(photo: MainPhoto) {
    setBusy(true);
    try { await selectMainPhoto(photo.path); onClose(); }
  ...
  async function addPhoto() {
    ...
    if (!uri) return;
    setBusy(true);
    try { await setBodyPhoto(uri); onClose(); }
...
const s = StyleSheet.create({
  backdrop: { flex: 1, justifyContent: 'flex-end', backgroundColor: '#0006' }, sheet: { backgroundColor: '#fff', borderTopLeftRadius: 28, borderTopRightRadius: 28, padding: 24, paddingBottom: 38 }, handle: { alignSelf: 'center', width: 34, height: 4, borderRadius: 3, backgroundColor: '#D9D9D5', marginBottom: 22 },
